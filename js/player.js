...
const mech = {
    ...
    displayHealth() {
		...
		////bruh
        id.style.width = Math.min(Math.floor(300 * mech.health),1500) + "px";
		...
		if (mech.health>5) {
			document.getElementById("field").innerHTML = mech.fieldUpgrades[mech.fieldMode].name+'(+'+Math.floor(mech.health*100-500)+' HP)'
		}
    },
    addHealth(heal) {
        if (!tech.isEnergyHealth) {
            mech.health += heal * simulation.healScale;
			mech.maxHealth+=Math.ceil(heal * simulation.healScale*tech.healthToMax*100)/100
    ...
    grabPowerUp() { //look for power ups to grab with field
        ...
        for (let i = 0, len = powerUp.length; i < len; ++i) {
            ...
            if (dist2 < mech.grabPowerUpRange2 &&
                ...
                !(((mech.health === mech.maxHealth)&&(tech.healthToMax==0)) && powerUp[i].name === "heal") &&
